...
void sleep(unsigned int seconds) {
	// TODO1: 实现一个阻塞方式的sleep库函数
	// 每10ms产生一次时钟中断,一秒需要100次
	int ticks = seconds * 100;
	for (int i = 0; i < ticks; i++) {
		syscall(3, 0, 0, 0, 0, 0);  // 设置timeFlag为0
		while (syscall(2, 0, 0, 0, 0, 0) == 0) {  // 等待timeFlag变为1
			// 轮询等待
		}
	}
}
